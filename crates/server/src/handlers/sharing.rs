...
pub(crate) async fn delete_folder(
    ...
    match to_bytes(body, BODY_LIMIT).await {
        ...
        {
            Ok(caller) => {
                match handlers::delete_folder(state, backend, caller, bytes)
                    .await
                {
                    Ok(result) => result.into_response(),
                    Err(error) => error.into_response(),
...
mod handlers {
    ...
    use sos_protocol::{
        ...
        UpdateFolderInviteRequest, UpdateFolderInviteResponse,
    ...
    pub(super) async fn create_folder(
        ...
        let packet = CreateSharedFolderRequest::decode(bytes).await?;
        ...
        // Empty response packet for now
        let packet = CreateSharedFolderResponse {};
