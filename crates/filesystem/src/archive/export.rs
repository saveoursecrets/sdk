...
async fn export_archive_buffer(
    ...
    let mut manifest = ManifestVersion1::new_v2();
    let mut writer = ZipWriter::new(Cursor::new(&mut archive));
    set_identity(&mut writer, &mut manifest, address, &identity).await?;
...
async fn set_identity(
    ...
    manifest.checksum = hex::encode(Sha256::digest(vault).as_slice());
    writer
...
async fn add_devices(
    ...
    manifest.devices = Some((vault_checksum, event_checksum));
...
async fn add_account_events(
    ...
    let event_checksum = hex::encode(Sha256::digest(events).as_slice());
    manifest.account = Some(event_checksum);
...
async fn add_file_events(
    ...
    let event_checksum = hex::encode(Sha256::digest(events).as_slice());
    manifest.files = Some(event_checksum);
...
async fn add_preferences(
    ...
    manifest.preferences = Some(checksum);
...
async fn add_remote_servers(
    ...
    manifest.remotes = Some(checksum);
