...
use sos_backend::{reducers::FolderReducer, BackendVaultAccess};
...
impl LocalAccount {
    ...
    pub(crate) async fn open_vault(
        ...
        {
            let storage =
                self.storage.as_ref().ok_or(StorageError::NoStorage)?;
    ...
    pub(crate) async fn build_search_index(
        ...
        let storage = self.storage.as_mut().ok_or(StorageError::NoStorage)?;
        let mut writer = storage.write().await;
...
impl Account for LocalAccount {
    ...
    async fn change_account_password(
        ...
        let mut output = BackendVaultAccess::new_vault(vault);
