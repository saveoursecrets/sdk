...
async fn pairing_account_name() -> Result<()> {
    ...
    primary_device
        .owner
        .rename_account(new_name.to_string())
        .await?;
    ...
    // Check account names match
    let device1_name = {
        let data = primary_device.owner.account_data().await?;
        data.account.label().to_owned()
    };
    let device2_name = {
        let data = enrolled_account.account_data().await?;
        data.account.label().to_owned()
    };
    assert_eq!(new_name, &device1_name);
    assert_eq!(new_name, &device2_name);
