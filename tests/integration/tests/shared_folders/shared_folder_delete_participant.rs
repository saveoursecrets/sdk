...
/// Tests creating a shared folder and having a participant
/// delete the folder. After a participant has deleted a
/// shared folder it should still be available to the owner
/// and other participants.
#[tokio::test]
async fn shared_folder_delete_participant() -> Result<()> {
    const TEST_ID: &str = "shared_folder_delete_participant";
    ...
    let mut recipients = vec![recipient1];

    // Perform a search to find the target recipient
    let results = account1
        .owner
        .search_recipients(&origin, "two".to_owned(), None)
        .await?;
    recipients.extend(results.into_iter());
    ...
    // Participant deletes the shared folder
    account2
        .owner
        .delete_shared_folder(&origin, shared_folder.id())
        .await?;

    // Immediately not available to the participant
    assert!(!account2
        .owner
        .list_folders()
        .await?
    ...
    // The shared folder is still accessible to the owner
    assert!(account1
        .owner
        .list_folders()
        .await?
    ...
    // Check the secret is still accessible to the owner
    assert!(account1
        .owner
