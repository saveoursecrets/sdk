...
impl Gatekeeper {
    ...
    pub fn label(&self) -> Result<String> {
        if let Some(private_key) = &self.private_key {
            if let Some(meta_aead) = self.vault.index().meta() {
                let meta_blob = xchacha20poly1305::decrypt(private_key, meta_aead)?;
    ...
    pub fn set_meta(&mut self, meta_data: MetaData) -> Result<()> {
        if let Some(private_key) = &self.private_key {
            ...
            let meta_aead = xchacha20poly1305::encrypt(private_key, &meta_blob)?;
    ...
    pub fn set_secret(
        ...
        if let Some(private_key) = &self.private_key {
            ...
            let secret_aead = xchacha20poly1305::encrypt(private_key, &secret_blob)?;
