...
impl<W: WalProvider + Send + Sync + 'static> ChangesListener<W> {
    ...
    async fn connect(cache: Arc<RwLock<FileCache<W>>>) -> Result<()> {
    ...
    #[async_recursion(?Send)]
    async fn delay_connect(cache: Arc<RwLock<FileCache<W>>>) -> Result<()> {
        loop {
